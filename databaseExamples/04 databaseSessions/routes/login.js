  ...
  router.post('/login', (req, res) => {
    if (!req.body.name || !req.body.password) {
      ...
      if (users.checkUser(req.body.name, req.body.password)) {
        if (!sessionStorage.addSession(req.sessionID, req.body.name,
          req.session)) {
          if (sessionStorage.getUser(req.sessionID) === req.body.name) {
            ...
            req.session.destroy();
            res.render('info',
              errorMessage('Error, please try again.', '/login'));
          }
        } else {
        ...
        res.render('info',
          errorMessage('You are not allowed to use this site','/',
            'Access denied'));
      }
    }
  });
...
function errorMessage(info='Error', url='/', title='Login',
  mode='error', delay=2) {
  return{title, info, url, delay, mode};
}

function statusMessage(info='Status', url='/', title='Login',
  mode='status', delay=1) {
  return{title, info, url, delay, mode};
}
